...
def main():
    ...
    if (request.method == "POST"):
        ...
        X = pd.DataFrame([[name,temp_list1[0],temp_list2[0],temp_list3[0],workexp,text,""]], columns = ["name","pref1","pref2","pref3","workexp","resume","model_category"])
        ...
        applicants.loc[applicants["username"] == username,["name","pref1","pref2","pref3","workexp","resume","model_category"]] = [name,temp_list1[0],temp_list2[0],temp_list3[0],workexp,text,last[0]]
        print(applicants.tail())
...
@app.route("/indexout", methods=['GET','POST'])  # this sets the route to this page
def indexout():	
    global username
    print(username)
...
@app.route("/registerout", methods=['GET','POST'])  # this sets the route to this page
def registerout():	
    global applicants
    email = request.form.get("email")
    password = request.form.get("password")
    ...
    if not usernames is None and email in usernames:
        ...
        username = email
        X = pd.DataFrame([[None,None,None,None,None,None,None,username,password]], columns = ["name","pref1","pref2","pref3","workexp","resume","model_category","username","password"])
...
@app.route("/register", methods=['GET','POST'])  # this sets the route to this page
def register():	
